...
export const registerUser = async (first_name: string, last_name: string, email: string, password: string, gender:string) => {
    ...
    try {
        ...
        if(emailExists){
            ...
            const query = 'INSERT INTO macrofit_tracker.users (user_id, activity_level, age, first_name,daily_caloric_intake, last_name, email, height, weight, password, gender) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
            ...
            const height = 0;
            const weight = 0;
            const params = [user_id, activity_level, age, first_name, daily_caloric_intake, last_name, email, height, weight, hashedPassword, gender];
